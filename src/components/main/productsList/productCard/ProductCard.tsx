...
const ProductCard = (props: ProductInterface) => {
	...
	return (
		<article className='text-sm md:text-xl w-[12rem] md:w-[15rem] p-2 md:p-4 border-2 border-gray/50 rounded-xl'>
			<div className='flex items-center justify-between mb-2 '>
				<span className='rounded-r-3xl rounded-l-md bg-green text-white p-1 md:p-2'>
					{props?.discountPrice}
				</span>

				<button
					onClick={() => addToFavorites(props)}
					className='border-2 border-gray/50 rounded-full flex items-center justify-between gap-2 text-black p-1 md:p-2'>
					<GoHeart />
				</button>
			</div>
